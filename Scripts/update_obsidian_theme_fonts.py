...
def build_stack_block(*, edenia_cjk_families: list[str]) -> str:
    ...
    return "\n".join(
        [
            MARK_STACK_BEGIN,
            "/* Hangul/Kana/Yi before CJK. Kana/yi stack: q/qv/qh/t/h/base.",
            "   Digraphs need one face (Obsidian post-processor / HTML pin).",
            "   CJK lists FE00-FE0F. */",
            "body {",
...
def write_plugin(faces: list[dict]) -> None:
    ...
    #
    # Digraph pin: CSS stacks can only list one segment face safely (Blink
    # picks fonts per codepoint → FE00 overlays full glyphs). Reading /
    # Live Preview post-process wraps *minimal* half clusters
    # (ZWSP? tile FE00 tile, or single tile+slice) in a span with the
    # matching `edenia … h` family. Do NOT treat all CJK as run glue —
    # that pinned whole paragraphs to `… h` and blew String.fromCodePoint.
    main_js = f"""const {{ Plugin, PluginSettingTab, Setting, Platform, normalizePath }} = require("obsidian");
...
function pinEdeniaDigraphs(root, settings) {{
  ...
  for (const textNode of nodes) {{
    ...
    if (!raw || !hasHalfSliceMark([...raw].map((ch) => ch.codePointAt(0)))) continue;
    ...
    for (const part of parts) {{
      if (part.family) {{
        ...
        span.style.fontFamily =
          "'" + part.family + "'" + (base !== part.family ? ", '" + base + "'" : "");
